...
class ImportadorInteligenteCFDI {
    ...
    /**
     * Detecta y extrae complemento de pagos
     */
    private function extraerComplementoPagos($contenidoXML) {
        ...
        if (preg_match('/<pago10:Pagos[^>]*>.*?<\/pago10:Pagos>/s', $contenidoXML, $matches)) {
            ...
            if (preg_match_all('/<pago10:Pago[^>]*([^>]*)>/s', $complemento, $pagosMatches)) {
                foreach ($pagosMatches[1] as $pagoAttrs) {
                    $pago = [];
                    ...
                    foreach ($campos as $campo => $columna) {
                        if (preg_match('/' . $campo . '\s*=\s*["\']([^"\']*)["\']/', $pagoAttrs, $matches)) {
            ...
            if ($file->isFile() && strtolower($file->getExtension()) === 'xml') {
                ...
                if ($count % 100 == 0) {
                    ...
                    echo sprintf(
                        ...
                        $count, $totalArchivos, ($count / $totalArchivos) * 100,
                        $this->stats['insertados'], $this->stats['errores'],
                        $this->stats['version_33'], $this->stats['version_40'],
                        $this->stats['complementos_pago']
                    );
                }
