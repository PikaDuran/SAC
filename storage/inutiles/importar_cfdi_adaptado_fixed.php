...
function insertCfdi($pdo, $data)
{
        ...
        foreach ($data['conceptos'] as $c) {
            ...
            if (!empty($c['impuestos'])) {
                foreach ($c['impuestos'] as $imp) {
                    insertImpuesto($pdo, $cfdi_id, $concepto_id, $imp);
                }
            }
        }
    }
    ...
    if (!empty($data['impuestos'])) {
        foreach ($data['impuestos'] as $imp) {
            insertImpuesto($pdo, $cfdi_id, null, $imp);
        }
    }
    ...
    // Guardar campos extras en cfdi_complementos
    ...
    // Insertar complementos restantes en cfdi_complementos
...
function insertImpuesto($pdo, $cfdi_id, $concepto_id, $impData) {
    $sql = "INSERT INTO cfdi_impuestos (cfdi_id, concepto_id, tipo, impuesto, tipo_factor, tasa_cuota, base, importe) VALUES (?,?,?,?,?,?,?,?)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([
        $cfdi_id,
        $concepto_id,
        $impData['Tipo'] ?? null,
        $impData['Impuesto'] ?? null,
        $impData['TipoFactor'] ?? null,
        $impData['TasaOCuota'] ?? null,
        $impData['Base'] ?? null,
        $impData['Importe'] ?? null
    ]);
}
...
function insertSpecializedComplements($pdo, $cfdi_id, $data) {
    ...
    // Insertar Pagos (adaptado a nuestra estructura)
    if (isset($data['complementos']['Pagos'])) {
        $pagos = $data['complementos']['Pagos'][0] ?? [];
        if (isset($pagos['Pago'])) {
            foreach ($pagos['Pago'] as $pago) {
                $sql = "INSERT INTO cfdi_pagos (cfdi_id, version, fecha_pago, forma_pago, moneda, tipo_cambio, monto, num_operacion, rfc_emisor_cuenta_ordenante, nombre_banco_extranjero, cuenta_ordenante, rfc_emisor_cuenta_beneficiario, cuenta_beneficiario) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([
                    $cfdi_id,
                    $pagos['Version'] ?? '2.0',
                ...
                // Insertar DocumentoRelacionado (adaptado a nuestra estructura)
...
if (isset($xmlPaths) && !empty($xmlPaths)) {
    $pdo = getDatabase();
        ...
        if (isset($data['error'])) {
            ...
            if ($stmtCheck->rowCount() > 0) {
                ...
                try {
                    ...
                    $err++;
                    $msg = date('Y-m-d H:i:s') . " | ERROR_INSERCION | $xmlFile | " . $e->getMessage() . "\n";
                }
            }
        }
    ...
    echo "  Errores: $err\n";
    echo "  Log de auditoría: $auditLog\n";
}
?>
