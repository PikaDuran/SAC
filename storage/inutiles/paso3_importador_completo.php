...
class ImportadorCFDICompleto
{
    ...
    private function procesarArchivoXML($archivo)
    {
        ...
        // Guardar la ruta del archivo para la inserción
        $datos_cfdi['__archivo'] = $archivo;
    ...
    private function limpiarXML($xml_content)
    {
        ...
        // Limpiar caracteres de control problemáticos (no afecta acentos ni caracteres especiales)
        $xml_content = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', '', $xml_content);

        // Asegurar codificación UTF-8 para permitir acentos y caracteres especiales
        if (!mb_detect_encoding($xml_content, 'UTF-8', true)) {
            $xml_content = mb_convert_encoding($xml_content, 'UTF-8');
        }
    ...
    private function extraerDatosCFDI($xpath)
    {
        ...
        if ($timbre) {
            $datos['UUID'] = $timbre->getAttribute('UUID');
        }
    ...
    {
        // INSERTAR CON UUID para manejo completo
        $sql = "REPLACE INTO cfdi (
            ...
            :no_certificado, :certificado, :condiciones_pago, :subtotal,
            :descuento, :moneda, :tipo_cambio, :total, :tipo_comprobante,
            :metodo_pago, :lugar_expedicion, :confirmacion, NOW(), :ruta_xml
        )";

        // Calcular ruta relativa del XML
        $ruta_base = realpath(__DIR__);
        $ruta_completa = realpath($datos['__archivo'] ?? '');
        $ruta_relativa = $ruta_completa ? str_replace($ruta_base . DIRECTORY_SEPARATOR, '', $ruta_completa) : ($datos['__archivo'] ?? '');
    ...
    private function insertarImpuestosConcepto($concepto_id, $concepto, $xpath)
    {
        ...
        echo "DEBUG: Conceptos con {$traslados->length} traslados\n";
        ...
        echo "DEBUG: Conceptos con {$retenciones->length} retenciones\n";
    ...
    private function insertarTimbreFiscal($cfdi_id, $xpath)
    {
        ...
        $sql = "REPLACE INTO cfdi_timbre_fiscal_digital (
    ...
    private function insertarPagosV10($cfdi_id, $pagos, $xpath)
    {
        ...
        foreach ($pagos_list as $pago) {
            $sql = "REPLACE INTO cfdi_complemento_pagos_v10 (
    ...
    private function insertarPagosV20($cfdi_id, $pagos, $xpath)
    {
        ...
        foreach ($pagos_list as $pago) {
            $sql = "REPLACE INTO cfdi_complemento_pagos_v20 (
    ...
    private function extraerAtributos($elemento)
    {
        ...
        foreach ($elemento->attributes as $attr) {
            // Los valores pueden contener acentos y caracteres especiales, se almacenan tal cual
